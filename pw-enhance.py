...
if 'class="pw-dashboard"' in html or 'pricewatch:watchlist' in html:
    raise SystemExit('Enhancements already applied')
...
'''
html = html.replace('<div class="table-wrap" id="tableWrap">', dashboard + '<div class="table-wrap" id="tableWrap">', 1)

html = html.replace('<div class="pos-chip" id="risersOnly">Movers only</div>', '''<div class="pos-chip" id="risersOnly">Movers only</div>
    ...
    <div class="pos-chip" id="myTeamOnly">♙ My Team</div>''', 1)

# Team metadata and transfer momentum while preserving the visible abbreviation.
new_map = '''  function mapPlayers(data){
    var teams = {};
...
html, n = re.subn(r'  function mapPlayers\(data\)\{.*?\n  \}\n\n  function applyFilters', new_map, html, count=1, flags=re.S)
if n != 1: raise SystemExit('mapPlayers anchor not found')

# Keep old localStorage snapshots readable while adding full club names and transfer metadata.
...
html = html.replace('  function saveFavs(favSet){\n    try{ localStorage.setItem(STORE_KEY_FAVS, JSON.stringify(Array.from(favSet))); }catch(e){}\n  }', '  function saveFavs(favSet){\n    try{ localStorage.setItem(STORE_KEY_FAVS, JSON.stringify(Array.from(favSet))); }catch(e){}\n  }' + insert, 1)
html = html.replace('    moversOnly: false,\n    favoritesOnly: false,', '    moversOnly: false,\n    favoritesOnly: false,\n    watchlistOnly: false, risingOnly: false, fallingOnly: false, differentialsOnly: false, myTeamOnly: false,\n    watchlist: loadWatchlist(), team: null, events: [], currentGameweek: null,', 1)

new_filter = '''  function applyFilters(list){
  ...
  function sortList'''
html, n = re.subn(r'  function applyFilters\(list\)\{.*?\n  \}\n\n  function sortList', new_filter, html, count=1, flags=re.S)
if n != 1: raise SystemExit('applyFilters anchor not found')
...
new_render = '''  function render(){
  ...
  function escapeHtml'''
html, n = re.subn(r'  function render\(\)\{.*?\n  \}\n\n  function escapeHtml', new_render, html, count=1, flags=re.S)
if n != 1: raise SystemExit('render anchor not found')

html = html.replace('      var mapped = mapPlayers(data);\n      var changed = detectChanges(state.players, mapped);', '      state.events = data.events || [];\n      state.currentGameweek = currentEvent(state.events);\n      var mapped = mapPlayers(data);\n      var changed = detectChanges(state.players, mapped);', 1)
html = html.replace('var trimmed = players.map(function(p){ return [p.id,p.name,p.team,p.pos,p.gw1,p.now,p.total,p.event,p.own,p.status]; });', 'var trimmed = players.map(function(p){ return [p.id,p.name,p.team,p.pos,p.gw1,p.now,p.total,p.event,p.own,p.status,p.teamName,p.transfersIn,p.transfersOut,p.netTransfers,p.points,p.form,p.epNext,p.minutes]; });', 1)
html = html.replace('return { id:a[0], name:a[1], team:a[2], pos:a[3], gw1:a[4], now:a[5], total:a[6], event:a[7], own:a[8], status:a[9] };', 'return { id:a[0], name:a[1], team:a[2], pos:a[3], gw1:a[4], now:a[5], total:a[6], event:a[7], own:a[8], status:a[9], teamName:a[10] || a[2], transfersIn:a[11] || 0, transfersOut:a[12] || 0, netTransfers:a[13] || 0, points:a[14] || 0, form:a[15] || 0, epNext:a[16] || 0, minutes:a[17] || 0 };', 1)

old_listener = '''  tbody.addEventListener("click", function(e){
    var favBtn = e.target.closest(".fav-btn");
    if (!favBtn) return;
    var id = parseInt(favBtn.getAttribute("data-fav-id"), 10);
    if (state.favs.has(id)) state.favs.delete(id);
    else state.favs.add(id);
    saveFavs(state.favs);
    render();
  });'''
new_listener = '''  function detailCard(label,value){ return '<div class="pw-detail-card"><span>'+escapeHtml(label)+'</span><b>'+escapeHtml(value)+'</b></div>'; }
