...
const newTeam = `  async function loadMyTeam(){
    ...
    var button=document.getElementById('pwLoadTeam');button.disabled=true;button.textContent='Loading…';
    try{
      var res=await fetchWithTimeout('/.netlify/functions/team?id='+encodeURIComponent(id),10000);
      if(!res.ok) throw new Error('Team endpoint HTTP '+res.status);
      var data=await res.json();
      if(!data || !Array.isArray(data.picks) || !data.picks.length) throw new Error(data&&data.error?data.error:'No picks returned');
      ...
      render();
    }catch(e){showToast('Couldn\\'t load team: '+(e&&e.message?e.message:'check Team ID'));}
