...
self.addEventListener("fetch",function(e){
  var url=e.request.url;if(isExternalData(url))return;
  if(isEnhancementScript(url)){
    e.respondWith(caches.match(e.request).then(function(cached){return fetch(e.request,{cache:"no-store"}).then(function(res){if(res.ok){var copy=res.clone();caches.open(CACHE).then(function(c){c.put(e.request,copy);});}return res;}).catch(function(){return cached;});}));return;
  }
  var isPageRequest=e.request.mode==="navigate"||url.indexOf("index.html")!==-1||url.endsWith("/");
  if(isPageRequest){
    e.respondWith((async function(){var cached=await caches.match(e.request);try{var preload="";if(e.preloadResponse)preload=await e.preloadResponse;var fresh=preload||await fetch(e.request,{cache:"no-store"});if(fresh.ok&&e.request.method==="GET"){var copy=fresh.clone();caches.open(CACHE).then(function(c){c.put(e.request,copy);});}return fresh;}catch(err){return cached||caches.match("./index.html");}})());return;
  }
  e.respondWith(caches.match(e.request).then(function(cached){return cached||fetch(e.request).then(function(res){if(e.request.method==="GET"&&res.ok){var copy=res.clone();caches.open(CACHE).then(function(c){c.put(e.request,copy);});}return res;}).catch(function(){return cached;});}));
});