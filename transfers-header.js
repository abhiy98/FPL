(function(){
  "use strict";
  if(location.pathname!=="/transfer"&&location.pathname!=="/transfers"&&location.pathname!=="/transfers.html"&&location.pathname!=="/transfers/index.html")return;
  ...
  function build(){
    if(document.getElementById("transferHeader"))return;
    var style=document.createElement("style");
    style.textContent=`
      body{padding:0!important}
      .transfers-header{padding:calc(env(safe-area-inset-top,0px) + 14px) 16px 12px;background:linear-gradient(180deg,#37003c,#0f021e 130%);border-bottom:1px solid rgba(255,255,255,.09);color:#f4eef6;box-sizing:border-box;position:relative;z-index:90}
      ...
      .transfers-menu{position:absolute;top:calc(100% + 6px);right:16px;width:185px;padding:6px;border:1px solid rgba(255,255,255,.16);border-radius:10px;background:#2f0334;box-shadow:0 12px 28px rgba(0,0,0,.45);display:none}
    ...
    header.innerHTML=`<div class="transfers-header-row"><div class="transfers-brand"><div class="transfers-mark">AY</div><h1>FPL AY Transfer</h1></div><div class="transfers-actions"><div class="transfers-live"><span class="transfers-live-dot"></span><span>LIVE</span></div><button class="transfers-menu-btn" type="button" aria-label="More options" aria-expanded="false">⋮</button></div><div class="transfers-menu" role="menu"><button type="button" data-action="refresh">Refresh</button><button type="button" data-action="add">Add team</button><button type="button" data-action="change">Change team</button><button type="button" data-action="remove" class="danger">Remove team</button><button type="button" data-action="price" class="nav">Price Change</button></div></div>`;
    ...
    if(window.MutationObserver){
      var observer=new MutationObserver(function(){
        ...
        if(pitch&&pitchBtn&&pitchBtn.classList.contains("active")){pitch.classList.remove("hidden");pitch.style.display="flex";if(list)list.classList.add("hidden");}
      });
    ...
    setTimeout(function(){
      var pitch=document.querySelector(".pitch-wrap");
      var pitchBtn=document.getElementById("pitchBtn");
      if(pitch&&pitchBtn&&pitchBtn.classList.contains("active")){pitch.classList.remove("hidden");pitch.style.display="flex";}
    },800);
  }
